...
class MLPRender_PE_Combine(torch.nn.Module): # position(x,y) -> positional encodding + ngp_result -> final_feature
    def __init__(self, inChanel, output_channel=16, pospe=6, featureC=64):
        super(MLPRender_PE_Combine, self).__init__()
...
class MLPRender_PE_Res_Tiny(torch.nn.Module): # position(x,y) -> positional encodding + ngp_result -> final_feature
    def __init__(self, inChanel, output_channel=16, pospe=6, featureC=64):
...
class TensorVMSplit(TensorBase):
    def __init__(self, aabb, gridSize, device, **kargs):
        ...
        self.extra_mlp = [MLPRender_PE_Combine(2+self.feature_num1+16,16).to(device),MLPRender_PE_Combine(2+self.feature_num1+16,16).to(device),MLPRender_PE_Combine(2+self.feature_num1+16,16).to(device)]
    ...
    def compute_densityfeature(self, xyz_sampled):
        ...
        for idx_plane in range(len(self.density_line)):
            ...
            ngp_fea = self.extra_mlp[idx_plane](normed_coordinate,ngp_out,ngp_line_point.T) + ngp_out
            #Each plane needs a unique hashtable to store its feature!!!
            #print(ngp_out.shape,ngp_line_point.shape)
            ngp_feature = ngp_feature + torch.sum(ngp_fea.T * ngp_line_point,dim=0)
            '''
